...
CASES = [
    ...
    # ===============================================================
    # 軸38〜40: ★★★【賢魂】をスペル枠へ落とす経路(裁定318)
    #
    #   ★★★<b>この3軸は 72 の追補で足した。</b>マスターの不具合報告
    #   (「スペルのエリアにドラッグしても賢魂が使えない」)を調べたところ、
    #   <b>70 はこの経路を1件も測っていなかった</b> ——
    #   70 の落とし先の検査が使う手札はミニオン・スペル・ウェポンの3枚で、
    #   <b>賢魂を持つカードが1枚も居なかった</b>。
    #   ★裁定318 の中心(「落とし先が【賢魂】かどうかの宣言を兼ねる」)そのものが穴だった。
    #   ★★<b>実装は壊れていなかった</b>が、壊れても誰も赤くしない状態だった ——
    #     70 の教訓「空文」の親戚である(書いてあるが、測られていない)。
    # ===============================================================
    ("落とし先が SPELL でも賢魂として扱わない(裁定318 の中心を消す)", BATTLE_JS,
     "    const asSoul = zone === 'SPELL' && card.soulCost != null && card.type !== 'SPELL';\n"
     "    if (d.from === 'TABOO') {",
     "    const asSoul = false;\n"
     "    if (d.from === 'TABOO') {",
     "verify", "", "賢魂を持つミニオンをスペル枠へ落とすと play-soul へ飛ぶ"),

    ("賢魂で払えてもスペル枠を落とし先に入れない(掴んでも落とせなくなる)", BATTLE_JS,
     "    if (p.soulAffordable && (view.phase === 'MAIN' || view.phase === 'SUB')) zones.push('SPELL');",
     "    if (false) zones.push('SPELL');",
     "verify", "", "【賢魂】を持つカードはスペル枠を落とし先に持つ"),

    # ★★<b>出口ごとに当てる</b>(71 の教訓)。賢魂の出口から return を落とすと、
    #   進化のときだけ<b>そのまま素材の分岐まで流れる</b> ——
    #   賢魂として唱えたのに「素材にできるミニオンの上に落としてください」と言われる
    ("賢魂の出口から return を落とす(進化だけ素材の分岐まで流れる)", BATTLE_JS,
     "    if (asSoul) {\n"
     ...
     "        beginSelection('play-soul', d.index, card.soulTargets, { manaIndexes: [] });\n"
     "    }",
     "verify", "", "賢魂を持つ<b>進化</b>をスペル枠へ落としても素材を尋ねない"),
]
