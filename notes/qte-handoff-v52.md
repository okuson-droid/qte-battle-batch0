...
**★★★46a はテストが本体である。マスターの `mvn test` が唯一の答え合わせになる。**
サンドボックスから Maven Central へ到達できない(403)ため、Java を触るバッチは
**1バッチ = マスターの1往復**である。だから 46 を 46a / 46b に割った(裁定160)。
...
- **★★★Maven Central へ到達できない(403)。`mvn test` もコンパイルもできない。**
  `javac -proc:none` による構文走査まではできる(型解決は不可)。
  **★46 以降は毎バッチが Java である。バッチを小さく刻み、テストを先に書くこと。**
...
- **★★`mvn test` を回す。** 期待: `CardTextKeywordsTest` 11件 / `CardIdMappingTest` 9件が
  すべてパスし、**`CardMasterLoadTest` のこれまで赤だった1件が緑になる**。
- Eclipse で **refresh(F5)** → コンパイルが通ること。
...
- **★★Maven Central の許可リスト追加**(会話で提案済み・未対応)。
  46〜57 は全12バッチが Java である。これが通れば往復が桁で減る。
