...
## ★★★ 設計書16 訂正(このバッチの納品後、同チャットで反映。v2.3 → v2.4)

**設計書 4-5(ドロップ先)・4-5-1・4-5-2 の3が定めていた「場を離れても数値は保持する」は撤回する。**
正しくは次のとおりである。**マスターが設計書16本体を v2.4 として更新済み**
(`batch16-manual-mode-design-v2_4.md`)。

> **ミニオンゾーン(FIELD)または装備品(WEAPON)の直下から離れる瞬間、数値は印刷値に戻る。**
> 「離れる」とは、移動先が別のゾーンになる場合と、進化の素材になる場合の両方を指す。
> 同じ種類のゾーンへ移す(FIELD → FIELD、WEAPON → WEAPON)場合は、
> 場に居続けている扱いなので戻さない。**ウェポンも v2.4 で対象に含まれた**(2-3-1)。

修正箇所は本文2-3・2-3-1にまとめてある。テストは31件
(旧26件 + FIELD離脱で3件差し替え・追加 + WEAPON離脱で2件追加)まで拡張し、
`javac` + リフレクション実行で全件通過を確認済みである。
...
- **★★設計書16 訂正: FIELD・WEAPON を離れると印刷値へ戻る(v2.4)。** 当初は「移動では
  数値に触らない」で納品したが、同チャットでマスターから2段階の訂正が入った。素材になるときも、
  場や装備先から他ゾーンへ移るときも、独立した個体としての履歴は切れて印刷値に戻る(2-3・2-3-1)。
...
- **★この環境で型検査と実行検証ができた。** Lombok と Spring の jar は取れないが、
  既存クラスの公開APIを写したスタブを書いて `javac` を通し、テスト31件を実際に走らせた(3-2)。
  訂正後の再検証(FIELD 分・WEAPON 分の2段階)も同じ手順で行った。
...
#### 2-3-1. ★★ v2.4: ウェポンも同じ規則に含めた

当初は「場から他のゾーンに移動したとき」という指示を文字どおり FIELD だけに絞って納品したが、
同チャットで**ウェポンも含めるとマスターから裁定が出た**。
...
条件を「移動元が FIELD」から「移動元が FIELD または WEAPON」に広げ、
比較対象も「移動先が FIELD 以外」から「移動元と移動先が異なるゾーン」に一般化した。
これにより次がすべて同じ1行の条件で説明できる。

| 移動元 → 移動先 | 結果 |
|---|---|
| FIELD → 別のゾーン(墓地・手札等) | 印刷値へ戻る |
| FIELD → FIELD(相手席へ置き直す) | 場に居続ける扱いなので戻らない |
| WEAPON → 別のゾーン(墓地・手札等) | **印刷値へ戻る(v2.4 で追加)** |
| WEAPON → WEAPON(装備し直し) | 戻らない(同じゾーン内の位置変更) |

FIELD → WEAPON のような通常は起きない組み合わせも、
「移動元が対象ゾーンで、移動先がそれと異なる」の一般則にそのまま従い、印刷値へ戻る。
特別扱いを増やしていない。
...
4. さらに AssertJ スタブに実際の検証を実装し、`ManualGameService` の
   ...
   **テスト31件をリフレクションで実行して全件通過を確認した。**
   (設計書16 訂正を反映した再検証を含む。旧仕様のテスト1件を新仕様に書き換え、
   FIELD 離脱の新規3件・WEAPON 離脱の新規2件を追加した)
...
| ウェポン | 2 | **★装備を外すと印刷値へ戻る(v2.4) / 装備し直しても数値は変わらない** |
...
| ★装備を外す | `/move` `{..., toZone:'TRASH'}`(WEAPON上のウェポンを墓地へ) | **attack が印刷値に戻ること(v2.4)** |
...
これは場に居続ける扱いなので数値を保つ。**同じ規則はウェポン(WEAPON)にも適用される**
(v2.4)。装備を外すと ATK は印刷値へ戻り、装備し直すだけなら戻らない。
</details>
