...
- **★`move` は数値に一切触らない。** 設計書 4-5 の「印刷値で初期化」は
  デッキ読み込み時に既に満たされており、ここで書き換えると 4-5-2 の
  「帯から1枚抜いてもタイルの数値は変えない」と衝突する(2-3)。
  傷んだまま場に戻したものを戻す口として `stat-reset` を1本足した。
...
- **★この環境で型検査と実行検証ができた。** Lombok と Spring の jar は取れないが、
  既存クラスの公開APIを写したスタブを書いて `javac` を通し、テスト26件を実際に走らせた(3-2)。
...
### 2-3. ★ `move` は数値に一切触らない — 設計書の2箇所を両立させる

設計書には数値の初期化について、一見ぶつかる記述が2つある。
...
17b の設計解説 2-2 も「帯から素材を抜いてミニオンゾーンの空き枠へ落とせば、**その数値のまま**場に戻る」
と書いている。移動先が同じ FIELD なのに、片方は初期化、片方は保持である。

**結論: `move` はどの移動でも数値に触らない。** 両立する。

1. 前者は既に満たされている。デッキ読み込み時に `ManualCardInstance.of(master)` が
   印刷値を入れており(17b)、**手札から場へ出す最頻の経路では何もしなくても印刷値になる**。
   「空欄なら空欄のまま」という括弧書きは、この生成時の挙動そのものの説明である。
2. 後者は数値に触らないことでしか満たせない。
3. そもそも「場に出た = 新品」は裁定である。移動元のゾーンで条件分岐すると、
   同じ操作が経路によって違う結果を返すことになり、5-1 の原則に反するうえ
   Undo で戻したときの理解も難しくなる。

**そのぶんの受け皿として `stat-reset`(印刷値に戻す)を1本足した。**
墓地から場へ戻したものを新品にしたい、といった場面は人間が明示的に押す。
設計書 5-3 の3・4「ATK / HP の変更」の一形態であり、新しい判断は増えていない。

進化(`evolve`)についても同じである。設計書 4-5-1 の
「重ねた直後の数値は、上に乗せた進化ミニオンの印刷値で初期化する」は
**下のミニオンの数値を引き継がない**という意味であり、
上に乗せるカードが自分の印刷値を持っている以上、何もしなくてもそうなる。
...
| 進化スタック | 5 | 枠 3 → 1 / 平坦(`+3` の上に重ねて `+4`) / 束ごと移動 /<br>**帯から抜いても数値が変わらない** / 素材なし / 素材の場所の検査 |
...
| ★進化 | `/evolve` `{occupantId, seat:'A', evolutionCardId:X, materialCardIds:[A,B,C]}` | **ミニオン枠が 3 → 1 に減り**、`stackSize` が4、`materials` が3件 |
| ★分解 | `/move` `{occupantId, cardIds:[materialsの中のinstanceId], toSeat:'A', toZone:'TRASH'}` | 素材が1件減る。**最上段の attack / hp は変わらない** |
...
2. **帯から素材を抜いても最上段の数値が動かないこと**(設計書 4-5-2 の3)。
...
<details>
<summary>Q2. 傷ついたミニオンを墓地から場へ戻した。HP は印刷値に戻るか</summary>

**戻らない。`move` は数値に一切触らない。**

設計書 4-5 の「空き枠に落とすと印刷値で初期化」は、デッキ読み込み時に
`ManualCardInstance.of(master)` が印刷値を入れていることで既に満たされている。
ここで移動先ごとに書き換えると、4-5-2 の3「帯から1枚抜いてもタイルの数値は変えない」と
正面から衝突する。

印刷値に戻したいときは `/stat-reset` を明示的に送る。**自動では絶対に呼ばれない。**
</details>
